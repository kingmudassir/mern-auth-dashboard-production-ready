...
const STATUS_CONFIG = {
  ...
  flagged: { label: 'Flagged', dot: '#E8622A', bg: 'rgba(232,98,42,0.08)', text: '#C4531F' },
...
const ALL_STATUSES = ['all', 'active', 'pending', 'flagged', 'sold', 'rejected', 'expired'];
...
function ConditionBadge({ condition }) {
  const cfg = CONDITION_CONFIG[condition] || CONDITION_CONFIG.used;
  return (
    <span
...
export default function AllCarListings() {
  ...
  const { mutate: flagListing, isPending: isFlagging } = useFlagListing();
  const { mutate: approveListing, isPending: isApproving } = useApproveListing();
  ...
  const [modal, setModal] = useState(null); // { type, listing }
  ...
  const stats = useMemo(() => {
    ...
    return { total: listings.length, active, pending, flagged, today };
  }, [listings]);
  ...
  const handleFlag = () => {
    const { listing } = modal;
    flagListing(listing._id, {
      onSuccess: () => {
        showToast(`"${listing.title}" has been flagged for review`);
        setModal(null);
      },
      onError: (e) => {
        showToast(e?.message || 'Failed to flag listing.', 'error');
        setModal(null);
      },
    });
  };
  ...
  const handleApprove = () => {
    ...
    approveListing(listing._id, {
      onSuccess: () => {
        showToast(`"${listing.title}" approved and is now live`);
        setModal(null);
      },
      onError: (e) => {
  ...
  return (
    <>
      ...
      <ConfirmModal
        ...
        title={`Delete "${modal?.listing?.title}"?`}
        message="This listing will be permanently removed and can't be recovered. The seller will lose all associated data and inquiries."
      ...
      <ConfirmModal
        ...
        title={`Flag "${modal?.listing?.title}"?`}
        message="This listing will be marked for review and hidden from public search until resolved. The seller will be notified."
      ...
      <ConfirmModal
        ...
        title={`Approve "${modal?.listing?.title}"?`}
        message="This listing will be approved and immediately visible to all users on the marketplace."
      ...
      <div className="cl-root">
        ...
        <div className="flex items-start justify-between mb-5 gap-4 flex-wrap">
          <div>
            ...
            <p className="cl-page-sub">
              Manage every vehicle listed on Paiyya — approve, flag, or remove
            </p>
          </div>
          {toast.msg && (
            <Toast
        ...
        <div className="cl-stat-grid mb-5">
          ...
          <StatCard
            ...
            label="Flagged"
            value={stats.flagged}
            accent={{ bg: 'rgba(232,98,42,0.08)', color: '#C4531F' }}
            sub={stats.flagged > 0 ? 'Needs review' : undefined}
          />
          <StatCard
            icon={TrendingUp}
            label="Added today"
          ...
          <>
                  ...
                  <div key={listing._id} className="cl-row">
                    ...
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <p className="cl-listing-name" title={listing.title}>
                    ...
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <div className="min-w-0">
                        <p className="cl-seller-name truncate" title={sellerName}>
                          {sellerName}
                        </p>
                        <p className="cl-seller-email truncate">{listing.seller?.email ?? '—'}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      {listing.fuelType && (
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="cl-detail-text capitalize">{listing.fuelType}</span>
                        </div>
                      )}
                    </div>
                    ...
                    <div>
                      ...
                      {(listing.reportCount ?? 0) > 0 && (
                        <div className="flex items-center gap-1 mt-1">
                          <Flag size={9} strokeWidth={2.5} style={{ color: '#C4531F' }} />
                          <span className="cl-report-count">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      {!isFlagged && (
                        <button
                          type="button"
                          onClick={() => setModal({ type: 'flag', listing })}
                  ...
                  <div key={listing._id} className="cl-mobile-card">
                    ...
                    <div className="flex items-start justify-between gap-2 mb-3">
                      <div className="flex items-center gap-2.5 min-w-0">
                        ...
                        <div className="min-w-0">
                          <p className="cl-listing-name truncate">
                            {listing.title ?? `${listing.year} ${listing.make} ${listing.model}`}
                          </p>
                          <p className="cl-listing-id">#{listing._id?.slice(-8)}</p>
                        </div>
                    ...
                    >
                      ...
                      <div>
                        ...
                        <p className="cl-meta-val truncate">{sellerName}</p>
                      </div>
                      <div>
                    ...
                    <div className="flex items-center justify-between mb-3">
                      ...
                      {(listing.reportCount ?? 0) > 0 && (
                        <div className="flex items-center gap-1">
                          <Flag size={9} strokeWidth={2} style={{ color: '#C4531F' }} />
                          <span className="cl-report-count">
                            {listing.reportCount} report{listing.reportCount !== 1 ? 's' : ''}
                          </span>
                        </div>
            ...
            <div className="cl-pagination mt-4">
              <p className="cl-pagination-info">
                ...
                of {filtered.length} listing{filtered.length !== 1 ? 's' : ''}
              </p>
              <div className="flex items-center gap-1.5">
                <button
                  type="button"
                ...
                {(() => {
                  ...
                  return pages.map((p, i) =>
                    p === '…' ? (
                      <span key={`ellipsis-${i}`} className="cl-page-ellipsis">
                        …
                      </span>
                    ) : (
                      <button
...
const STYLES = `
  ...
  .cl-page-title {
    ...
    color: #1A1523; letter-spacing: -0.035em;
    margin: 0 0 2px;
  }
  .cl-page-sub {
    font-size: 0.8rem; color: #8A8390; margin: 0;
  }
  ...
  @media (max-width: 1024px) { .cl-stat-grid { grid-template-columns: repeat(3, 1fr); } }
  @media (max-width: 600px)  { .cl-stat-grid { grid-template-columns: repeat(2, 1fr); } }

  /* ── Search ── */
  .cl-search {
    display: flex; align-items: center; gap: 10px;
    padding: 0 14px; height: 42px;
    border-radius: 12px; border: 1.5px solid #E8E3DC;
    background: #FFFFFF;
  ...
  .cl-search-input {
    ...
    font-size: 0.85rem; color: #1A1523;
  ...
  .cl-filter-btn {
    ...
    padding: 0 14px; height: 42px;
    border-radius: 12px; border: 1.5px solid #E8E3DC;
    background: #FFFFFF; color: #8A8390;
  ...
  @media (max-width: 640px) {
    .cl-tabs-wrap:not(.cl-tabs-open) { display: none; }
  }
  .cl-tabs {
  ...
  .cl-tab {
    ...
    font-family: 'DM Sans', sans-serif;
    color: #8A8390; background: transparent;
  ...
  .cl-tab-active {
    background: #FFFFFF;
    color: #1A1523;
    box-shadow: 0 1px 4px rgba(26,21,35,0.1);
  }
  .cl-tab-count {
  ...
  .cl-tab-active .cl-tab-count {
    background: rgba(108,60,225,0.1); color: #6C3CE1;
  }
  ...
  .cl-th {
    ...
    color: #C4BDD0; font-family: 'DM Sans', sans-serif;
    white-space: nowrap;
  }
  .cl-row {
    display: grid;
  ...
  /* Cell text */
  .cl-listing-name {
    font-size: 0.82rem; font-weight: 600; color: #1A1523;
    font-family: 'DM Sans', sans-serif; line-height: 1.2;
  }
  .cl-listing-id {
    font-size: 0.65rem; color: #C4BDD0;
    font-family: 'DM Sans', sans-serif; margin-top: 1px;
  }
  .cl-seller-name {
    font-size: 0.78rem; font-weight: 500; color: #1A1523;
    font-family: 'DM Sans', sans-serif; line-height: 1.2;
  }
  .cl-seller-email {
    font-size: 0.68rem; color: #8A8390;
    font-family: 'DM Sans', sans-serif;
  }
  .cl-price {
    font-size: 0.85rem; font-weight: 700; color: #1A1523;
    font-family: 'Syne', sans-serif; letter-spacing: -0.02em;
    margin-bottom: 2px;
  }
  .cl-detail-text {
    font-size: 0.7rem; color: #8A8390;
    font-family: 'DM Sans', sans-serif;
  }
  .cl-city {
    font-size: 0.75rem; color: #8A8390;
    font-family: 'DM Sans', sans-serif;
  }
  .cl-report-count {
    font-size: 0.65rem; font-weight: 600; color: #C4531F;
    font-family: 'DM Sans', sans-serif;
  }
  .cl-posted-ago {
    font-size: 0.75rem; font-weight: 500; color: #1A1523;
    font-family: 'DM Sans', sans-serif;
  }
  .cl-posted-date {
    font-size: 0.65rem; color: #C4BDD0;
  ...
  .cl-action-btn {
    ...
    border-radius: 8px;
    display: flex; align-items: center; justify-content: center;
    border: 1.5px solid #E8E3DC; background: #FAFAF9;
    color: #8A8390; cursor: pointer;
  ...
  .cl-meta-label {
    ...
    letter-spacing: 0.06em; color: #C4BDD0;
  ...
  .cl-page-ellipsis {
    ...
    font-size: 0.78rem; color: #C4BDD0;
    font-family: 'DM Sans', sans-serif;
  }
  ...
  .cl-empty-sub {
    font-size: 0.75rem; color: #8A8390; margin: 0;
    font-family: 'DM Sans', sans-serif;
  }
  .cl-clear-btn {
  ...
  .cl-spinner {
    ...
    border: 2.5px solid rgba(108,60,225,0.15);
    border-top-color: #6C3CE1; border-radius: 50%;
    animation: spin 0.7s linear infinite;
  }
  .cl-loading-text {
    font-size: 0.8rem; color: #8A8390;
    font-family: 'DM Sans', sans-serif;
  }
`;
